  ...
  try {
    ...
    if (turnCount < 5) {
      ...
      if (turnCount === 0) {
        ...
        systemPrompt = `당신은 CCTV 영상 속 인물을 분석하는 AI입니다. 사용자의 이전 답변을 바탕으로 꼬리에 꼬리를 무는 긴 질문을 해주세요. 
        ...
        5. 꼬리에 꼬리를 무는 긴 질문으로 구성 (2-3개의 연관된 질문을 하나로 묶어서)
        6. "그리고", "또한", "또는", "그렇다면" 등으로 질문을 연결
        7. 사용자가 여러 측면에서 상세히 답할 수 있도록 유도
        8. 사람마다 다른 분석 결과를 얻을 수 있도록 다양한 관점의 질문
        9. 쉬운 단어와 간단한 문장으로 질문해주세요
        10. 긴 질문이지만 자연스럽게 연결된 질문으로 구성해주세요:`;
